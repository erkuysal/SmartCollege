...
class AttendanceSession(models.Model):
    ...
    def get_status(self):
        ...
        if elapsed_time <= 600:  # First 10 minutes
            return "Present"
        elif elapsed_time <= 1200:  # Next 10 minutes
            return "Late"
        else:
            return "Closed"
...
class Attendance(models.Model):
    ...
    class Meta:
        unique_together = ('session', 'student')  # Prevent duplicate attendance entries

    def __str__(self):
        return f"{self.student.user.email} - {self.session.course.course_name} - {self.status}"
